...
class BartQAModel(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        val_data_file = os.path.join(self.input_dir, "{}.json".format("val"))

        result = self.metric.get_eval(val_data_file, predictions)
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        print("f1 score", result['F1'])
        print("EM", result["EM"])
        values = {'F1': float(result['F1']), 'EM': float(result["EM"]),
                  'TOTAL': int(result['TOTAL']), 'SKIP': int(result["SKIP"])}
